# -*- coding: utf-8 -*-
"""
Final Standalone Script: Bullish Trend Follower (with Capital Management)
...
This version includes capital management and position sizing.
"""
...
def prepare_data(data_5m, data_60m, tickers, nifty_ticker):
    ...
    if nifty_ticker not in data_60m:
        raise ValueError("Nifty 60m data not found.")
...
def run_backtest(data, ticker, capital, risk_perc):
    """Runs the backtest with capital management, looking ONLY for LONG trades."""
    trades = []
    in_trade = False
            ...
            if (long_crossover and candle[adx_col_name] > ADX_THRESHOLD and candle['Nifty_HTF_Uptrend'] and candle['RSI'] > RSI_LEVEL):
                ...
                if risk_per_share > 0:
                    ...
                    if shares > 0:
                        ...
                        trade = {'ticker': ticker, 'type': 'LONG', 'entry_time': candle['Datetime'], 'entry_price': entry_price,
                                 'shares': shares, 'stop_loss': stop_loss, 'take_profit': take_profit, 'capital_before': capital}
...
if __name__ == '__main__':
    try:
        ...
        for ticker, stock_data in all_stock_data.items():
            trades, current_capital = run_backtest(stock_data, ticker, current_capital, RISK_PERCENTAGE)
