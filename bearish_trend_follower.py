# -*- coding: utf-8 -*-
"""
Final Standalone Script: Bearish Trend Follower

This script backtests a trend-following strategy specialized for BEARISH trends.
It will ONLY take SHORT trades when the overall market trend is bearish.

This is one of the three final specialized strategy scripts.

To run, simply execute:
python3 bearish_trend_follower.py
"""
...
def prepare_data(data_5m, data_60m, tickers, nifty_ticker):
    """Prepares data for the bearish trend-following strategy."""
    prepared_data = {}
...
def run_backtest(data, ticker):
    """Runs the backtest, looking ONLY for SHORT trades."""
    trades = []
    in_trade = False

    print(f"\nRunning BEARISH backtest for {ticker}...")
    ...
    for i in range(1, len(data)):
        ...
        if in_trade:
            if candle['High'] >= trade['stop_loss'] or candle['Low'] <= trade['take_profit']:
                exit_price = trade['stop_loss'] if candle['High'] >= trade['stop_loss'] else trade['take_profit']
        ...
        if not in_trade:
            ...
            # Bearish Entry Condition: Only take SHORT trades if Nifty trend is also DOWN.
            if (short_crossover and candle[adx_col_name] > ADX_THRESHOLD and not candle['Nifty_HTF_Uptrend'] and candle['RSI'] < RSI_LEVEL):
                ...
                if risk > 0:
                    ...
                    trade = {'ticker': ticker, 'type': 'SHORT', 'entry_time': candle['Datetime'], 'entry_price': entry_price, 'stop_loss': stop_loss, 'take_profit': take_profit}
    return trades
